...
by the head materials. Normal and detail textures remain raw inputs; their packing
has not been translated into a native lighting model.
...
These PNG bakes use byte-normalized RGB arithmetic and nearest integer RGBA8
output. Source code enables `GL.sRGBWrite`; project color-space conversion,
GPU sampler precision, mipmaps, and render-target quantization are not reproduced.
...
disagree with these compiled pass states and must not override them. Draw shader
lighting, stencil-dependent effects, and specialized tear behavior are not yet
ported. Current native toon lighting remains approximate.
...
shadow variant 12. Native color, depth/shadow, and outline passes need consistent
coverage. The shader's null `_AlphaMask` default is white. `_MainTex.a` does not
...
partial/nonfinite channels. The four-pass source body-mask GPU probes also pass
with the extended vertex interface.
