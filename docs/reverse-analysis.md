...
append additional data. The current native reader returns the exact end offset
of the object section and does not claim to validate or interpret this remainder.
...
The raw reader retains distance X/Y and roll. Ikkoku's orbit camera cannot
represent every source camera exactly without expanding its model. A scene
camera object and saved viewport CameraData are different records.

`FKCtrl`, `IKCtrl`, `OIBoneInfo`, and `OIIKTargetInfo` were recovered for inspection.
The original bone group enum is a bit mask; the original records use numeric
keys, and IK target serialization inherits the bone record. Existing Ikkoku pose
deltas, named skeletons, and two-bone solver have not been proven behaviorally
equivalent. No solver replacement or full character import is claimed here.
...
- Explicit rejection of unsupported versions, characters (kind 0), routes
  (kind 4), unknown kinds, invalid UTF-8/lengths, negative counts, duplicate
  object keys, nonfinite floats, and truncated records.
- Limits of 256 MiB per input, 1 MiB per string, 100,000 objects per scene or
  entries per collection, and hierarchy depth 64.

The reader never follows paths in a source Pattern record, loads assets, executes
source code, or interprets a plugin trailer. PNG framing is checked but PNG CRC
and image pixels are not decoded. Caller-visible import success therefore means
the supported object section parsed; it does not establish complete-scene parity.

Regression fixtures are synthetic. They check multibyte seven-bit string lengths,
