...
One inspected male body renderer has **101 bone references but 100 inverse bind
matrices**. The inventory reports this mismatch; neutral export rejects it.
Truncating the array or inventing a bind matrix would conceal a real assembly
issue that still needs source-code analysis.
...
state-0 mesh has 148 references and 148 binds. See
[the source avatar report](reverse-head-rig.md).
...
- `meshes`: owning `node` and `skin` indices, `rendererEnabled`, local `positions`,
  `normals`, `tangents`, `uv0`, four-slot `joints` and `weights`, submesh indices,
...
The exporter rejects missing/external joints, mismatched bind counts, invalid
...
Use Python 3.12 with the pinned UnityPy dependency; the installed Python 3.14 could
not compile UnityPy 1.23.0. See the [pipeline README](../Tools/reverse/README.md)
...
inputs, never textures or unrelated text assets. Shader/material appearance,
runtime clothing rebinding, dynamics, and complete Maker assembly behavior are
separate work; this data establishes the rig and morph contracts.
