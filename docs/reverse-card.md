...
This is an import and preservation path. Saved Sideloader references can also be
checked against the mounted native mod library, with catalog matches and typed
asset dependencies reported separately. This lookup does not yet recreate a
card's hair, outfits, materials, modded asset selections, or all body proportions.
...
face/body versions are `0.0.2`; the installed hair version is `0.0.4`. The native
reader decodes the face and body records and keeps hair bytes opaque. The
...
differs from the ABMX plugin GUID. See [ABMX behavior and limits](reverse-abmx.md)
...
`appearanceApplied: false` for every record. Compatibility migrations, missing
mod substitutions, and managed plug-in execution remain unimplemented.
...
In Maker, choose **Open Source Rig…** and the recovered
`.local/reverse/rigs/source-avatar.json`, then **Import Source Card Settings…**
and an original-format character card. The settings adapter checks the card
against the explicitly selected assembly's sex, head and body-bone identity and
requires its 44-body/52-face shape contract. All applied shape values must be in
...
card, and shows scope diagnostics under **Imported card settings**. Existing
...
content profile for explicit appearance bindings; importing a card does not yet
apply its asset selections through that library.
...
Remaining work includes applying resolved source asset selections, compatibility
migrations and unsupported dependency conversion, alternate-head application,
standalone coordinate-card framing, and additional managed behavior adapters. Native rendering coverage is distinct from preserving
and editing the original card format.
