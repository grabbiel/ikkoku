...
event selection, ADV execution, menus, scene loading, and save serialization are
identified dependencies and remain outside this translation.
...
invalid original inputs. Period/name localization and script string parsing,
class-schedule bundle loading, and all event-dependent period branches remain
untranslated.
...
| Scheduled events | `FixEventScheduler`, `FixEventSchedule`, class schedules | Preserve first-uncompleted-event ordering, day thresholds, week/period filters, lesson-map checks, and matching wait-point layers. |
...
| ADV execution | `ADV.Program.Transfer`, `TextScenario`, `MainScenario`, command handlers and scenario assets | Recover typed commands and scenario decoding; implement execution, variables, branches, waits, and character/audio/camera bindings. |
...
| Animation | NPC/AI motion requests and character animation controllers | Connect recovered state requests to a native animator with source controller/clip/parameter semantics. |
...
The focused suite contains eight native tests, nine Python oracle tests, and five
optional CLI regression tests. Set `IKKOKU_INSPECTOR` to the absolute path of the
...
Native
oracle comparison checks every generated state field and daily-effect count,
