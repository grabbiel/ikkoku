...
The installed game uses Mono. `Tools/reverse/analysis/recover_managed.py` now
...
- [Gameplay](reverse-gameplay.md): day/period progression and action-clock state,
  with explicit effects owed to scene, NPC, ADV and save systems.
- [Animation](reverse-animation-playback.md): temporal blinking and expression
  timers; automatic blinking is connected to Maker.
- [Studio](reverse-studio-pose.md): FK target binding, FK/IK activation rules and
  absolute local-rotation evaluation on source rigs.
