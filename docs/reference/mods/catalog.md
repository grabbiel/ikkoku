...
Reviewed 2026-09-25. `SourceModCatalog` implements the bounded parser, identity
lookup and typed dependency report described here. Saved card resolver records
have a [separate native adapter](card-references.md); source version selection,
GUID migrations and ID-only fallback are still pending. This is a reference
contract, not a claim that every catalog entry is drawable. Current status and
follow-up tasks are in the [character/mod audit](../../component-audit/character-and-mods.md)
(CM-26–28, CM-46–50; CMT-09/10).
...
The source stream reader detects BOMs and uses decoder replacement for malformed byte sequences; it does not guess CP932. The package importer's RFC CSV/CP932 interpretation is analytical metadata;
`SourceModCatalog` therefore reparses the preserved raw bytes using the source
catalog contract. An unsupported encoding can be reported without altering those bytes.
...
Character-save hooks write the original `Slot` and attach resolver metadata through Extended Save; after saving/loading, resolution restores the appropriate runtime `LocalSlot`. Preserving only a temporary runtime integer therefore loses identity. Current [card Extended Save framing](../character/cards.md) and saved-reference
reading are implemented for bounded formats. Migration execution, runtime-ID
injection and writing changed resolver identities remain separate work.
...
usable in Maker. Saved [card resolver records](card-references.md) and their
actual serialized destinations are supported separately. `SourceMakerLibrary`
can consume matching converted selections, but the audited library contains no
mod-GUID geometry entries. Original automatic version selection, migration and
ID-only compatibility resolution remain unfinished.

To extend this layer, implement those resolution decisions against the existing
independent oracle, then add a mod-provided geometry/material fixture through
catalog lookup, Maker selection and edited-card preservation. Keep temporary
`LocalSlot` values out of persisted native identities. Acceptance details are
CMT-09/10 in the linked audit.
