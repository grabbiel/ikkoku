...
The retained kernel verification run passed five native tests and six Python
tests. These counts describe that focused evidence run, not the current total suite. The independent oracle uses world
...
A separate [schema-2 full-body adapter](../studio/full-body-ik.md) now implements
effector/chain coupling, spine/body pull and push, mapping constraints, iterative
solve order, bend constraints and Studio target binding. Its recovered-C# oracle
is independent of this kernel's tests. Direct Unity-player full-body comparison,
runtime solver hooks and the blocked source pose UI remain pending (`ST-T01`,
`ST-T05` in the audit). This kernel still rejects indirect or scaled chains; those
limits must not be generalized to the separate full-body adapter.
