...
Reviewed 2026-09-25. This describes the clothed normal-male reference assembly
and recorded source-space checks. The current Maker also supports registered
male heads 200/201, correction-table bone types and converted card-selected parts;
see [Maker assets](maker-assets.md) and the
[component audit](../../component-audit/character-and-mods.md) (CM-13, CM-16/17).
Commands run from the repository root. These measurements are saved evidence,
not a new verification run.
...
original card colors. Selected hair dynamics and source shader probes exist
separately, but Maker does not yet rebind dynamics for every card-selected hair
assembly and its normal preview still uses approximate toon lighting.
...
The recorded run passed, with maximum absolute error `5.44e-7` in matrices and `5.78e-7` in
vertices. These checks compare every retained node and selected vertex, not just
finite values.

The recorded recovered-C# body oracle run also passed all 844 cases with the male
...
and source bundle integrity. See [card appearance](card-appearance.md)
