...
`KoikatsuSceneReader.decodeDocument` now reads the full installed **1.0.4.2**
...
The result includes the staged pose/controller, deferred source effects and
diagnostics for missing saved bones and unsupported runtime systems. It does not
substitute native IK for FinalIK. A source IK-enabled record remains explicitly
unsupported for solved-pose parity. Stored bone positions/scales remain available
as source data; FKCtrl only overwrites local Euler rotation.
...
It restores supported female/head-00 card shapes, static ABMX and saved FK onto
the explicit local clothed reference avatar. Source object transforms retain
their exact quaternion, with consistent editable Euler fields. Unsupported
character variants, props, lights, cameras, routes and unresolved attachment
descendants remain named, unrendered tree entries. The compatibility panel lists
the remaining work; this preview does not claim to reproduce original appearance.
Scene camera/effects and character Animator/IK states are retained as source data
rather than implicitly applied to this preview.
...
converted objects release their cached previews. Unsupported original bytes remain
in the referenced file, not an edited source-scene export. Four integration tests
cover native card round trips, actual-avatar FK/shape transforms and finite Metal
preview bounds, stale references, unsupported cards and failed-load mesh cleanup.
...
the base scene. This is data retention, not edited original-scene serialization.
...
Remaining work includes original character appearance reconstruction, source
animation selection/evaluation, FinalIK and dynamics, guides/look-at, accessory
attachment assembly, moving routes, sound, scene effects, object-reference
remapping during imports, plug-in callbacks and edited-scene saving. The app must
report these gaps alongside any supported character preview rather than present
record parsing as complete scene compatibility.
