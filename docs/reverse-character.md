> This document preserves the initial channel/direct-setter investigation. The
> current runtime implements [all 44 body slots and complete recovered body
> destinations](reverse-body-shape.md), [all 59 head-00 operations](reverse-face-shape.md),
> and [normal male assembly](reverse-male.md). [Original card import/export](reverse-card.md)
> and [guarded appearance recipes](reverse-card-appearance.md) now use these records.
> See [rebuild status](REBUILD.md) for the assembled Maker and remaining limits.
...
normal female/male assemblies. Original card import, bounded appearance edits and
edited-card export are available; arbitrary card-selected assets, full material
parity and complete character animation remain separate work.
...
additive component corrections when the caller requests corrected bones. The
Maker card preview currently accepts standard-body cards only.
...
Maker imports supported normal male/female head-00/standard-body cards and exports
a new original-format copy. Unknown nested MessagePack values are not coerced
...
the three body `UpdateAlways` setters and all 59 head-00 destinations. This includes
...
The initial channel investigation ran six native tests, including interpolation across the 360° boundary,
