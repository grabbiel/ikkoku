...
Reviewed 2026-09-25. This guide covers scanning and mounting immutable **asset-mod
packages**. It is a usable, bounded pipeline with broader conversion still
pending; see the [mod overview](../reference/mods/overview.md) for the distinct
Maker, card, ABMX and managed-plugin paths, and the
[character/mod audit](../component-audit/character-and-mods.md) (CM-46–50,
CMT-09/10) for remaining work. Run the commands from the repository root.
...
This scanner preserves catalogs and produces a mount profile; it does not
resolve missing providers, apply catalog migrations or reproduce source conflict
priority. The [catalog adapter](../reference/mods/catalog.md) reports dependencies,
the [card resolver](../reference/mods/card-references.md) reads saved identities,
and the [ABMX adapter](../reference/mods/abmx.md) evaluates supported static records.
Those existing consumers do not make arbitrary newly scanned mods renderable.

Managed plugin packages use the separate [translation and execution path](../reference/mods/plugin-execution.md);
this library never runs original DLLs. Unsupported asset/conversion behavior
remains explicit in package diagnostics. No background rescan, general
mesh/material conversion or unified asset/managed-plugin library is installed.
Also retain the audit's Unicode identity caveat: exact resolver lookups use source
UTF-8 identity, but some package/profile grouping still uses Swift String equality.
Complete source ordinal-identity parity across the entire library is unfinished.
