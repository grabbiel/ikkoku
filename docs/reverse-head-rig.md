...
This stage uses the locally installed `p_cf_head_00` geometry and its source
...
5. Parent body and clothing prefabs under the character's `objTop`. Rebind their
   ...
   implementation should reject a missing mapping rather than invent one.
...
raw inputs; no original dynamic color composition has been applied. The source
...
All 111 cases passed. Maximum absolute error was `5.44e-7` in node matrices and
...
binds. Six assembled rest/face cases passed across 774 retained nodes, 21 parts
...
For source bind inspection before a native executable is available, use
