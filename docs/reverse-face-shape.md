# Female head shape destination stage

`SourceFaceShapePose` implements the 59 destination operations in the installed
game's female head shape updater. It consumes the raw Unity channel state from
`SourceShapeDomain`, which already samples the 52 face sliders from the local
head-00 curve contract. This adds destination mapping for the 48 operations that
...
Seven focused tests cover rest axes, explicit unit axes, distinct left/right
...
- This is the female head-00 shape stage. It does not implement male heads,
  alternate head contracts, expression blend shapes, gaze, blinking, hair,
  materials, or face animation.
