#!/usr/bin/env python3
"""Numerical Animator transforms from a fresh clothed isolated Unity player."""
...
def start(vm,output,source=None):
    ...
    source=source or Path(__file__).with_name('fixtures')/'OriginalAnimationProbe.cs'
    (output/'probe-source.cs').write_bytes(source.read_bytes())
    write_small(vm,root+r'\AnimationProbe.cs',source.read_bytes())
    result=retry(vm,"$ErrorActionPreference='Stop';$root="+ps_quote(root)+r''';$refs=@("$root\BepInEx\core\BepInEx.dll")+(Get-ChildItem "$root\CharaStudio_Data\Managed" -Filter '*.dll'|ForEach-Object {$_.FullName});& C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe /nologo /noconfig /nostdlib+ /target:library /out:"$root\BepInEx\plugins\IkkokuAnimationProbe.dll" /reference:$($refs -join ',') "$root\AnimationProbe.cs";if($LASTEXITCODE -ne 0){throw 'Compile failed'};if(Test-Path "$root\BepInEx\plugins\character"){Remove-Item "$root\BepInEx\plugins\character" -Recurse -Force}''')
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--vm',default='Windows 11');p.add_argument('--output',type=Path,default=ROOT/'.local/reverse/original-animation-probe');p.add_argument('--source',type=Path);p.add_argument('--collect',action='store_true');p.add_argument('--stop',action='store_true');a=p.parse_args()
